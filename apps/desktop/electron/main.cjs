...
app.whenReady().then(() => {
  ...
  window = new BrowserWindow({ width: 1440, height: 940, minWidth: 940, minHeight: 660, title: 'Niuery Agent', backgroundColor: '#f5f7fa', webPreferences: { preload: path.join(__dirname, 'preload.cjs'), contextIsolation: true, nodeIntegration: false, sandbox: true, webviewTag: true } });
  ...
  ipcMain.handle('terminal:start', async (event, cwd) => {
    ...
    const child = spawn('powershell.exe', ['-NoLogo', '-NoProfile', '-ExecutionPolicy', 'Bypass'], { cwd, windowsHide: true, stdio: ['pipe', 'pipe', 'pipe'] });
