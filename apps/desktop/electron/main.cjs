...
app.whenReady().then(() => {
  ...
  ipcMain.handle('worker:request', async (event, method, payload) => {
    if (event.sender !== window.webContents || !['hello','providers.list','history.list','events.list','run.start','run.cancel','approval.respond','workspace.diff','run.continue','artifact.undo'].includes(method)) throw new Error('请求不被允许。');
