...
(async () => {
  ...
  try {
    ...
    await expect(page.locator('.timeline-latest').first()).toHaveText('执行完成。');
    await page.locator('.timeline-title').first().click();
    await expect(page.locator('.turn-timeline .timeline-item')).toHaveCount(2);
